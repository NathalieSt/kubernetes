...
func main() {
	...
	name := shared.MatrixSignalBridge
	...
	meta := generator.GeneratorMeta{
		...
		ClusterUrl:    "signal-bridge.signal-bridge.svc.cluster.local",
		Port:          29334,
		Docker: &generator.Docker{
			Registry: "dock.mau.dev/mautrix/signal",
		...
		Flux: &kustomization.KustomizationSpec{
			...
			Path:    "./cluster/apps/matrix/signal-bridge",
			Prune:   true,
			Wait:    true,
			Timeout: "10m",
			DependsOn: []string{
